...
{
    public class Agg2_Shift_StepSvc
    {
        static Agg2_Shift_StepSvc()
        {
            Util.RegisterOracle();
        }
        ...
        /// <returns></returns>
        public static Agg2_Shift_Step Get(int agg2_Shift_Id, byte step)
        {
            if (step >= 3)
                throw new Exception($"Invalid Agg Step Number for Agg2_Shift Table - {step}");

            StringBuilder sql = new();
            sql.Append(GetSelect());
            sql.AppendLine($"WHERE ass.agg2_shift_id = {agg2_Shift_Id}");
        ...
        /// <returns></returns>
        public static Agg2_Shift_Step Get(DateTime report_Date, byte shift, byte step)
        {
            if (step >= 3)
                throw new Exception($"Invalid Agg Step Number for Agg2_Shift Table - {step}");
            StringBuilder sql = new();
            sql.Append(GetSelect());
            sql.AppendLine($"WHERE a2s.report_date = {MOO.Dates.OraDate(report_Date)}");
        ...
        /// <returns></returns>
        public static int UpdateFromShiftInput(Agg2_Shift_Step agRecord)
        {
            int recsUpdated = 0;
        ...
        /// <param name="ag2LineRecord"></param>
        /// <returns></returns>
        public static int UpdateFromShiftInput(OracleConnection conn, Agg2_Shift_Step agRecord)
        {
           

            StringBuilder sql = new();
            sql.AppendLine($"UPDATE tolive.agg2_shift_step");
        ...
        private static string GetSelect()
        {
            ...
            StringBuilder sql = new();
            sql.AppendLine("SELECT a2s.report_date, a2s.shift, ass.agg2_shift_id, ass.step, ROUND(ass.ball_hrs, 4) ball_hrs, ROUND(ass.bent_lbs, 4) bent_lbs,");
            ...
            sql.AppendLine($"FROM tolive.agg2_shift_step ass");
            sql.AppendLine($"INNER JOIN tolive.agg2_shift a2s");
            sql.AppendLine($"ON ass.agg2_shift_id = a2s.agg2_shift_id");
            return sql.ToString();
        }
        ...
        /// <returns></returns>
        private static Agg2_Shift_Step DataRowToObj(DataRow row)
        {


            Agg2_Shift_Step retObj = new();
