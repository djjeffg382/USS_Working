...
namespace MOO.DAL.ToLive.Services
{
    public class Agg3_DaySvc
    {
        ...
        public static Agg3_Day Get(DateTime ShiftDate)
        {
        ...
        public static List<Agg3_Day> GetAll(DateTime StartDate, DateTime EndDate)
        {
            StringBuilder sql = new();
            sql.Append(GetSelect());
        ...
        internal static string GetColumns(string TableAlias = "", string ColPrefix = "")
        {
            ...
            cols.AppendLine($"{ta}agg3_day_id {ColPrefix}agg3_day_id, {ta}report_date {ColPrefix}report_date, ");
            ...
            cols.AppendLine($"ROUND({ta}ball_hrs,4) {ColPrefix}ball_hrs, {ta}bent_lbs {ColPrefix}bent_lbs, ");
            cols.AppendLine($"ROUND({ta}bent_lbs_lton,4) {ColPrefix}bent_lbs_lton, ROUND({ta}bin_6_pct,4) {ColPrefix}bin_6_pct, ");
            cols.AppendLine($"ROUND({ta}bin_7_pct,4) {ColPrefix}bin_7_pct, ROUND({ta}coal_stons,4) {ColPrefix}coal_stons, ");
            cols.AppendLine($"ROUND({ta}dd1_inh2o,4) {ColPrefix}dd1_inh2o, ROUND({ta}fuel_oil_gals,4) {ColPrefix}fuel_oil_gals, ");
            cols.AppendLine($"ROUND({ta}fuel_oil_tank_lvl,4) {ColPrefix}fuel_oil_tank_lvl, ROUND({ta}gas_mscf,4) {ColPrefix}gas_mscf, ");
            cols.AppendLine($"ROUND({ta}grate_hrs,4) {ColPrefix}grate_hrs, ROUND({ta}lines_running_sum,4) {ColPrefix}lines_running_sum, ");
            cols.AppendLine($"ROUND({ta}mbtu_lton,4) {ColPrefix}mbtu_lton, ROUND({ta}mbtus,4) {ColPrefix}mbtus, ");
            cols.AppendLine($"ROUND({ta}mo_to_day_pel_ltons,4) {ColPrefix}mo_to_day_pel_ltons, ");
            ...
            cols.AppendLine($"ROUND({ta}recl_flux_bal_ltons,4) {ColPrefix}recl_flux_bal_ltons, ");
            cols.AppendLine($"ROUND({ta}tank_slurry_2_1_pct,4) {ColPrefix}tank_slurry_2_1_pct, ");
            cols.AppendLine($"ROUND({ta}tank_slurry_2_2_pct,4) {ColPrefix}tank_slurry_2_2_pct, ROUND({ta}wood_stons,4) {ColPrefix}wood_stons, ");
        ...
        private static string GetSelect()
        {
            ...
            sql.AppendLine("FROM tolive.agg3_day");
            return sql.ToString();
        }



        internal static Agg3_Day DataRowToObject(DataRow row, string ColPrefix = "")
        {
            Agg3_Day RetVal = new();
            RetVal.Agg3_Day_Id = (int)row.Field<decimal>($"{ColPrefix}agg3_day_id");
            ...
            RetVal.Bin_6_Pct = row.Field<decimal?>($"{ColPrefix}bin_6_pct");
            RetVal.Bin_7_Pct = row.Field<decimal?>($"{ColPrefix}bin_7_pct");
            ...
            RetVal.Recl_Flux_Bal_Ltons = row.Field<decimal?>($"{ColPrefix}recl_flux_bal_ltons");
            RetVal.Tank_Slurry_2_1_Pct = row.Field<decimal?>($"{ColPrefix}tank_slurry_2_1_pct");
            RetVal.Tank_Slurry_2_2_Pct = row.Field<decimal?>($"{ColPrefix}tank_slurry_2_2_pct");
