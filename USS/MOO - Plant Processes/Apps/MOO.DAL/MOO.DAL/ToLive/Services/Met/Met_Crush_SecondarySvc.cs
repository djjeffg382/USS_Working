...
{
    public static class Met_Crush_SecondarySvc
    {
        static Met_Crush_SecondarySvc()
        {
            Util.RegisterOracle();
        }
        ...
        /// <returns></returns>
        public static List<Met_Crush_Secondary> Get(DateTime StartDate, DateTime EndDate, byte StartCrusherNbr, byte EndCrusherNbr)
        {
            ...
            List<Met_Crush_Secondary> retVal = new();
        ...
        /// <returns></returns>
        public static List<Met_Crush_Secondary> GetMonthSummary(DateTime RollDate, byte StartCrusherNbr, byte EndCrusherNbr)
        {
            List<Met_Crush_Secondary> retVal = new();
            //limit for loop to only be 1-15 if start and line number are outside that range
            for (byte i = Math.Max(StartCrusherNbr, (byte)1); i <= Math.Min(EndCrusherNbr, (byte)15); i++)
            {
        ...
        /// <returns></returns>
        public static Met_Crush_Secondary GetMonthSummary(DateTime RollDate, byte LineNumber)
        {
            StringBuilder sql = new();
            sql.AppendLine("DECLARE");
            sql.AppendLine("    RollDate Date;");
            sql.AppendLine("    Line Number;");
            sql.AppendLine("    mcs met_crush_secondary%rowtype;");
            sql.AppendLine("BEGIN");
            sql.AppendLine("    RollDate := :RollDate;");
            sql.AppendLine("    Line := :LineNumber;");
            sql.AppendLine("    tolive.met_roll.met_crush_secondary_m(RollDate, Line, mcs);");
            ...
            sql.AppendLine("            ROUND(mcs.crush_tons,4) crush_tons, ROUND(mcs.crush_hours,4) crush_hours,");
            sql.AppendLine("            ROUND(mcs.crush_kwh,4) crush_kwh, ROUND(mcs.metal_dly_hours,4) metal_dly_hours,");
            sql.AppendLine("            ROUND(mcs.sched_hours,4) sched_hours");
        ...
        private static string GetSelect()
        {
            ...
            sql.AppendLine("FROM tolive.met_crush_secondary");
            return sql.ToString();
        }


        public static int Insert(Met_Crush_Secondary obj)
        {
        ...
        public static int Insert(Met_Crush_Secondary obj, OracleConnection conn)
        {
            
            StringBuilder sql = new();
            sql.AppendLine("INSERT INTO tolive.Met_Crush_Secondary(");
        ...
        public static int Update(Met_Crush_Secondary obj)
        {
            using OracleConnection conn = new(MOO.Data.GetConnectionString(Data.MNODatabase.DMART));
            conn.Open();
        ...
        public static int Update(Met_Crush_Secondary obj, OracleConnection conn)
        {
            StringBuilder sql = new();
            sql.AppendLine("UPDATE tolive.Met_Crush_Secondary SET");
        ...
        private static Met_Crush_Secondary DataRowToObject(DataRow row)
        {
            Met_Crush_Secondary RetVal = new();
